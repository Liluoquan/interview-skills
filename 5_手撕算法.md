...
1. [124. 二叉树中的最大路径和](https://leetcode.cn/problems/binary-tree-maximum-path-sum/)
...
2. [988. 从叶结点开始的最小字符串](https://leetcode.cn/problems/smallest-string-starting-from-leaf/)
...
1. 判断是有向图，还是无向图（**决定建图方式**）
2. 权值是否为负数
3. 找起点到所有点的路径还是起点到终点的路径（**是否需要提前返回**）
4. 判断要求路径的最大代价还是最小代价（**决定优先队列的排序方式**）

**例题**

1. [1631. 最小体力消耗路径](https://leetcode.cn/problems/path-with-minimum-effort/)
2. [743. 网络延迟时间](https://leetcode.cn/problems/network-delay-time/)
3. [1514. 概率最大的路径](https://leetcode.cn/problems/path-with-maximum-probability/)
4. [2290. 到达角落需要移除障碍物的最小数目](https://leetcode.cn/problems/minimum-obstacle-removal-to-reach-corner/)
...
struct State {
    ...
    State(int i_, int j_, int dist_) : i(i_), j(j_), dist(dist_) {}
};
...
public:
    int minimumEffortPath(vector<vector<int>>& heights) {
        ...
        vector<int> distTo(m * n, INF);
        priority_queue<State, vector<State>, cmp> minHeap;
        minHeap.push({0, 0, 0});

        while (!minHeap.empty()) {
            auto [x, y, curDist] = minHeap.top();
            minHeap.pop();
            
            if (x == m - 1 && y == n - 1)   return curDist;

            // 当前位置不是最优就可以不用继续了
            if (curDist > distTo[x * n + y])    continue;
            ...
            for (auto[dx, dy] : dirs) {
                ...
                // 计算当前位置的代价
                int nextDist = max(curDist, abs(heights[nx][ny] - heights[x][y]));
                if (nextDist < distTo[nx * n + ny]) {  // 更新全局最优的代价
                    distTo[nx * n + ny] = nextDist;
                    minHeap.push({nx, ny, nextDist});
                }
            }
        }
...
1. [238. 银河英雄传说 - AcWing题库](https://www.acwing.com/problem/content/description/240/)
...
3. 路径压缩的过程
   - 找到根结点root
   - 计算父节点到上一个父节点的距离d[p[x]],同时进行路径压缩(路径压缩后，上一父节点即root)
   - d[x]初始值是x到p[x]的距离，更新d[x]值，即d[x] = d[x] + d[p[x]]

![a89bb0e9b6da8ae5cb676d00ccaacb5.png](https://cdn.acwing.com/media/article/image/2020/04/04/7416_92657e3476-a89bb0e9b6da8ae5cb676d00ccaacb5.png)
...
class UnionFind {
public:
    UnionFind(int n) : fa(n), dist(n), sz(n) {
        for (int i = 0; i < n; ++i) {
            fa[i] = i;
            dist[i] = 0;
            sz[i] = 1;
...
private:
    vector<int> fa;
    vector<int> dist; // 节点到根节点的距离
    vector<int> sz;   // 节点所在连通块的大小
}; 
...
**用于计算无向图同将所有点连接起来的最小代价**

一般来说，我们都是在**无向加权图**中计算最小生成树的，所以使用最小生成树算法的现实场景中，图的边权重一般代表成本、距离这样的标量。

1. Prim算法，该算法以顶点为单元，与图中边数无关，比较适合于稠密图
2. Kruskal算法，该算法以边为单元，时间主要取决于边数，比较适合于稀疏图
...
1. 构建所有权重边，这道题每两个点之间都可以看做是有一条边，所以，一共是 n*(n-1)/2n∗(n−1)/2 条边；

2. 按权重边排序，这步直接排序即可；
3. 使用并查集来构造最小生成树，并查集主要用来判断两个是否是已经连通的状态；
...
1. 统计每个点的相邻点及其权重，这里每个点的相邻点是其他所有的点，所以，这一步可以省略；

2. 任选一顶点，我们假设选择 0 号点；
3. 使用小顶堆构造最小生成树，每加入一个点，就把它相邻点加入，每次弹出权重最小的点；
...
public:
    int minCostConnectPoints(vector<vector<int>>& points) {
        ...
        while (!minHeap.empty()) {
            ...
            for (int j = 0; j < n; ++j) {
                if (!vis[j])    minHeap.push({getDist(points[i], points[j]), j});
            }
        }
...
4. [背包问题求方案数](https://www.acwing.com/problem/content/11/)（求01背包中的最优方案数，具体就是多开一个dp数组记录方案数）
4. [背包问题求具体方案](https://www.acwing.com/problem/content/12/)（多开一个数组记录路径）
...
1. [多重背包问题 I](https://www.acwing.com/problem/content/4/)
2. [多重背包问题 II](https://www.acwing.com/problem/content/5/)
...
- **排列问题**
  1. [377. 组合总和 Ⅳ](https://leetcode.cn/problems/combination-sum-iv)（完全背包）
  1. 科大讯飞笔试（待整理）
- **组合问题**
  1. [518. 零钱兑换 II](https://leetcode.cn/problems/coin-change-2)（完全背包）
  1. 
...
int getResult(vector<int>& nums, int target) {
    int n = nums.size();
    // dp[i][j]：j个物品组成和为i的组合数
    vector<vector<int>> dp(target + 1, vector<int>(n + 1, 0));
    dp[0][0] = 1;
    for (int i = 0; i < n; ++i) {  // 先遍历物品（组合数）
        for (int j = target; j >= 0; --j) {  // 倒叙遍历（0-1背包）
            for (int k = 1; k <= n; ++k)
            if (j - nums[i] >= 0)	dp[j][k] += dp[j - nums[j]][k - 1];
        }
    }
    
    int res = 0;
    for (int j = 0; j < n; ++j) {
        if (dp[target][j] == 0)	continue;
        res += dp[target][j] * helper(j);
    }
...
int getResult(vector<int>& nums, int target) {
    ...
    for (int i = 0; i < n; ++i) {  // 先遍历物品（组合数）
        for (int j = target; j >= 0; --j) {  // 倒叙遍历（0-1背包）
            if (j - nums[i] >= 0)	dp[j] += dp[j - nums[j]];
        }
    }
...
int getResult(vector<int>& nums, int target) {
    ...
    dp[0] = 1;		// base case
    for (int i = 1; i <= target; ++i) {  // 先遍历背包（排列数），正序遍历背包（完全背包）
        for (int j = 0; j < n; ++j) {
...
int getResult(vector<int>& nums, int target) {
    ...
    dp[0] = 1;   // base case
    for (int i = 0; i < n; ++i) {  // 先遍历物品（组合数）
        for (int j = 0; j <= target; ++j) {  // 正序遍历背包（完全背包）
...
如何之前当前位置的dp范围的话，是可以预先记录范围内所有的位置的，不需要从头开始找。

也就是说，dp数组可以不是数组，可以是哈希表！！！

**例题：**

1. [6138. 最长理想子序列](https://leetcode.cn/problems/longest-ideal-subsequence/)
2. [1218. 最长定差子序列](https://leetcode.cn/problems/longest-arithmetic-subsequence-of-given-difference/)
2. [300. 最长递增子序列](https://leetcode.cn/problems/longest-increasing-subsequence/)（利用二分进行优化）
2. [最长递增子序列—牛客网 (nowcoder.com)](https://www.nowcoder.com/questionTerminal/30fb9b3cab9742ecae9acda1c75bf927)（输出具体的最长递增子序列）
2. [673. 最长递增子序列的个数](https://leetcode.cn/problems/number-of-longest-increasing-subsequence/)
2. [646. 最长数对链](https://leetcode.cn/problems/maximum-length-of-pair-chain/)
...
vector<int> helper(vector<int>& nums) {
    int n = nums.size();
    vector<int> dp(n, 1);   // 以 nums[i] 为结束元素的最长递增子序列长度
...
```c++
// 1218. 最长定差子序列（用哈希表作为dp数组，提高查找效率）
...
```cpp
// 673. 最长递增子序列的个数（维护两个数组记录不同的状态）
int findNumberOfLIS(vector<int>& nums) {
    int n = nums.size();
    // dp[i]：以nums[i]结尾的最长递增子序列长度
    // cnt[i]：以nums[i]结尾的最长递增子序列的数量
...
主要思想就是看当前字符是否匹配，再看上一个字符是否匹配。

**例题：**

1. [97. 交错字符串](https://leetcode.cn/problems/interleaving-string/)
...
1. [32. 最长有效括号](https://leetcode.cn/problems/longest-valid-parentheses)
2. [678. 有效的括号字符串](https://leetcode.cn/problems/valid-parenthesis-string/)
...
1. [6137. 检查数组是否存在有效划分](https://leetcode.cn/problems/check-if-there-is-a-valid-partition-for-the-array/)（判断好当前位置成立的条件）
...
1. [纸牌博弈__牛客网 (nowcoder.com)](https://www.nowcoder.com/questionTerminal/7036f62c64ba4104a28deee98a6f53f6)
