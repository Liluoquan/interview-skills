...
然后就是状态转移方程的不同：不考虑 $nums[i]$ 的情况下，填满容量为 $j - nums[i]$ 的背包，有 $dp[j - nums[i]]$ 种方法。

1. 组合问题：$dp[j] = dp[j]+dp[j - nums[i]]$，
...
1. [873. 最长的斐波那契子序列的长度](https://leetcode.cn/problems/length-of-longest-fibonacci-subsequence/)（确定第三个数，双指针找前两个）
...
### 2、64匹马，8个赛道，找出前四名最少需要比赛多少场？（腾讯、字节）
...
### 3、如何快速找出1000个瓶子中的一瓶毒药（微软、腾讯pcg）
